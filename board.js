(() => {
  ...
  style.textContent = `
    ...
    body.boardMode .shell{max-width:none;padding-left:18px;padding-right:18px}
    ...
    .workshopBoardTools{display:grid;grid-template-columns:minmax(260px,520px) auto;gap:12px;align-items:center;margin:14px 0}
    .workshopBoardTools .btn{min-height:54px;width:auto;white-space:nowrap}
    .workshopBoardStats{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:10px;margin:10px 0 20px}
    .workshopBoardStat{min-width:0;background:#fff;border:1px solid #d0d5dd;border-radius:14px;padding:12px 14px;box-shadow:0 1px 3px rgba(16,24,40,.04)}
    .workshopBoardStat b{display:block;font-size:24px;line-height:1}.workshopBoardStat small{display:block;color:#667085;margin-top:7px;font-size:13px;line-height:1.2}
    .workshopKanban{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:14px;align-items:start;padding:2px 0 24px;min-height:55vh;overflow:visible}
    .workshopColumn{min-width:0;width:auto;background:#f8fafc;border:1px solid #cfd6df;border-top:4px solid #667085;border-radius:18px;padding:12px;box-shadow:0 2px 8px rgba(16,24,40,.05)}
    ...
    .workshopColumnHead{position:static;background:#fff;border:1px solid #e4e7ec;border-radius:12px;display:flex;justify-content:space-between;gap:8px;align-items:center;padding:10px 11px;font-weight:900;margin-bottom:12px;box-shadow:0 1px 3px rgba(16,24,40,.04);line-height:1.2}
    .workshopColumnCount{min-width:29px;height:29px;border-radius:999px;background:#f2f4f7;display:grid;place-items:center;font-size:13px;flex:0 0 auto}
    .workshopDropzone{min-height:118px;border-radius:12px;padding:1px}
    .workshopCard{background:#fff;border:1px solid #cfd6df;border-radius:15px;padding:13px;margin:12px 0;box-shadow:0 3px 10px rgba(16,24,40,.07);cursor:pointer;touch-action:manipulation;overflow:hidden}
    .workshopCard:hover{border-color:#84adff}.workshopCardTop{display:flex;align-items:center;justify-content:space-between;gap:10px}.workshopPlate{font-size:19px;font-weight:900;letter-spacing:.2px;min-width:0;overflow:hidden;text-overflow:ellipsis}
    .dragGrip{display:inline-flex;align-items:center;gap:5px;border:1px solid #cbd5e1;background:#f8fafc;color:#344054;border-radius:10px;padding:8px 10px;font-weight:900;cursor:grab;touch-action:none;user-select:none;font-size:12px;flex:0 0 auto}.dragGrip:active{cursor:grabbing}.dragGripIcon{font-size:16px;line-height:1}
    .workshopCard small{display:block;color:#667085;margin-top:8px;line-height:1.42;overflow-wrap:anywhere}.workshopCard select{width:100%;min-height:44px;font-size:14px;padding:7px 9px;margin-top:12px}
    ...
    .workshopGhost{opacity:.25}.workshopChosen{box-shadow:0 12px 28px rgba(16,24,40,.22);transform:scale(1.015);border-color:#2563eb}.workshopDrag{opacity:.92}
    ...
    @media(max-width:1500px) and (min-width:901px){
      .workshopKanban{grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}
      .workshopBoardStats{grid-template-columns:repeat(3,minmax(0,1fr))}
    }
    @media(max-width:900px) and (min-width:701px){
      .workshopKanban{grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}
      .workshopBoardStats{grid-template-columns:repeat(3,minmax(0,1fr))}
    }
    @media(max-width:700px){
      ...
      .workshopKanban{grid-template-columns:1fr;gap:16px;min-height:0;padding:0 0 18px}
      .workshopColumn{width:100%;max-width:none;min-width:0;padding:11px;border-radius:16px}
      .workshopColumnHead{margin-bottom:11px;padding:10px 12px}
        ...
        <div class="workshopDropzone" data-status="${st}">
          ${rows.length ? rows.map(o => {
            ...
            return `<article class="workshopCard" data-id="${o.id}" data-vehicle-id="${o.viatura_id}" onclick="openVehicleDetailEnhanced('${o.viatura_id}')">
              <div class="workshopCardTop"><div class="workshopPlate">${esc(v.matricula||'—')}</div><button class="dragGrip" type="button" aria-label="Arrastar viatura" title="Arrastar para outra fase" onclick="event.stopPropagation()"><span class="dragGripIcon">⋮⋮</span><span>Arrastar</span></button></div>
  ...
  function initBoardDragDrop(){
    ...
    document.querySelectorAll('#workshopKanban .workshopDropzone').forEach(zone => {
      const sortable = new Sortable(zone, {
        ...
        onStart: evt => {
          evt.item?.classList.add('workshopChosen');
        },
        onEnd: async evt => {
          evt.item?.classList.remove('workshopChosen');
